 ...
 */
export function generateRightdownConfig(
 ...
 */
export function generateRightdownConfigContent(
  config: OutfitterConfig
): string {
  ...
  lines.push(
    `  // Generated by @outfitter/baselayer with strictness: ${config.strictness}`
  );
  lines.push(
    '  // Docs: https://github.com/outfitter-dev/monorepo/tree/main/packages/rightdown'
  );
  lines.push('');
  ...
  Object.entries(mdConfig).forEach(([key, value], index, arr) => {
    ...
    switch (key) {
      ...
      case 'customRules':
        ...
        lines.push(
          '  // Paths to JavaScript files exporting markdownlint rules'
        );
        lines.push(
          `  "${key}": ${JSON.stringify(value, null, 2).replace(/\n/g, '\n  ')}${isLast ? '' : ','}`
        );
